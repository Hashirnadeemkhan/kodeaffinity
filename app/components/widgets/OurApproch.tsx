...
const OurApproch = () => {
  useEffect(() => {
    const animateNumbers = () => {
      ...
      const numberElements = document.querySelectorAll<HTMLElement>(".count-up");
        ...
        (entries, observer) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              const start = 0;
              const end = parseInt(el.getAttribute("data-end") || "0"); // Provide a default value if null
              ...
              const countUpInstance = new CountUp(el, end, options);
